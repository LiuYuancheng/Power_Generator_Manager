...
PLC_FLAG = False # flag whether we want to control PLC from the serialCtrl program. 
...
WIN_H = 400 if PLC_FLAG else 320    # window height.
...
class UIFrame(wx.Frame):
    ...
    def __init__(self, parent, id, title):
        ...
        wx.Frame.__init__(self, parent, id, title, size=(300, WIN_H))
        ...
        self.fieldlList = [] # data control field list.
        ...
        if len(portList)> 0:
            ...
            try:
                baudRate = 115200
                self.serComm = serial.Serial(self.serialPort, baudRate, 8, 'N', 1, timeout=1)
            except:
                print("Error: Serial connection: serial port open error.")
                #return None
        else:
            self.serComm = None
            print("Warning: No connection can be used!")
        # Try to setup the PLC connection.
        if PLC_FLAG:
            # connect to the PLC
